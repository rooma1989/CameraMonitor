...
from PySide6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QFormLayout, QLabel,
    QLineEdit, QPushButton, QCheckBox, QSpinBox, QSizePolicy)
...
class Decoder(QThread):
    ...
    def __init__(self, url, parent=None, max_width=1920):
    ...
    def run(self):
        ...
        try:
            while not self.cancel.is_set():
                ...
                try:
                    options = {'rtsp_transport':'tcp', 'rw_timeout':'4000000'} if live else {}
...
class PlayerWindow(QDialog):
    ...
    def __init__(self, device, parent=None, credential_store=None):
        super().__init__(parent)
        self.device = device
        ...
        title = QLabel(f'{device.name or device.model or "摄像头"} · {device.ip}')
    ...
    def set_busy(self, value):
        for widget in (self.username,self.password,self.mode,self.channel,self.manual,self.connect_button):
    ...
    def start_decoder(self, stream):
        ...
        self.rendering = True
        self.set_busy(True)
        self.decoder = Decoder(url, self, max_width=self.preview_width)
