...
- 自动、4 / 9 画面，以及 6 / 9 / 10 / 12 / 15 路“一大多小”布局，最多同时查看 15 台摄像头。
- 画面拉伸铺满、不裁切；环绕布局以边缘对齐为优先，部分窗口比例会变化。
- 拖动交换画面与主位并记住顺序，双击可放大。全屏只保留画面和名称，按 Esc 返回。
...
4. 顶部选择“一大多小”布局，拖动画面调整主位，F11 进入全屏、Esc 返回。名称颜色和位置在各摄像头设置中调整。
