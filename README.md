...
- 等分布局按钮全部直接显示在顶部；一大多小布局在下拉菜单选择。10 格采用横竖各 5 小格、共用右下角的环绕结构，周边小画面尺寸一致。
- 左侧设备采用紧凑横排缩略图；搜索后获取预览，点击看大图。需已有账号密码，正在播放的设备优先复用现有画面。
- 全屏退出需密码（初始 `000000`），通过“大屏密码”修改或验证当前密码后恢复初始密码。全屏只允许放大/还原、拖拽排序；这是应用内锁，不阻止操作系统强制结束进程。
- 视频区域保持 16:9，原始画面完整拉伸至该比例、不裁切；屏幕比例不匹配时外围留空。
- 拖动交换画面、主位或空位并记住位置，单击放大、再次单击返回。全屏显示画面和叠加名称，可设置顶部机构名称，按 Esc 返回。
