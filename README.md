...
当前支持 **RTSP 视频预览**，暂不支持 RTMP、音频和录像。自动发现需要摄像头开启兼容协议；如果已知 IP 但搜索不到，可点击左侧 **按 IP 添加**，输入摄像头 IPv4 地址进行定向探测，识别成功后填写连接账号密码。定向添加不会清空当前设备列表，未收到回复时会提示检查网络和 ONVIF 设置。也可在已发现设备的连接设置中手动填写 RTSP 地址。
